...
import { Swords, Trophy, Plus, Minus, Flag, Activity, Users, Crown, ChevronRight, Trash2 } from 'lucide-react';

export default function Arena({ session }) {
  const [subTab, setSubTab] = useState('marcador');
  
  // Estados - Marcador Rápido
  ...
  const [tourneyPlayers, setTourneyPlayers] = useState(''); // Textarea para meter os nomes
  ...
  useEffect(() => {
    ...
    // Auto-refresh do placar de 5 em 5 segundos
  ...
  async function criarTorneio(e) {
    ...
    // 1. Baralhar os jogadores (Sorteio)
    let players = playersList.sort(() => Math.random() - 0.5);
    
    // 2. Achar a potência de 2 mais próxima (4 ou 8)
    const target = players.length <= 4 ? 4 : 8;
    while (players.length < target) players.push("BYE"); // Jogador fantasma (Passagem direta)

    // 3. Criar os Rounds
    const numRounds = Math.log2(target); // Se target=8, 3 rounds. Se 4, 2 rounds.
    const rounds = [];
    ...
    for (let r = 0; r < numRounds; r++) {
      ...
      for (let m = 0; m < numMatches; m++) {
        if (r === 0) {
          // Ronda 1 já tem os jogadores preenchidos
          roundMatches.push({ p1: players[m*2], p2: players[m*2+1], winner: null });
        } else {
          // Rondas futuras começam vazias
    ...
    // 4. Resolver os BYEs automaticamente
  ...
  async function avancarVencedor(tourneyId, currentRounds, roundIdx, matchIdx, vencedor) {
    ...
    // Avançar para a próxima ronda se existir
    if (roundIdx + 1 < newRounds.length) {
      setSlot(newRounds, roundIdx + 1, matchIdx, vencedor);
    }

    // Verificar se o torneio acabou (se foi a última ronda)
    ...
    <div style={{ padding: '10px', paddingBottom: 'calc(130px + env(safe-area-inset-bottom))' }}>
      ...
      <div className="card" style={{ textAlign: 'center', background: 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)', color: 'white' }}>
        <h2 style={{ margin: '0 0 5px 0', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px' }}>
          <Swords size={26} color="#38bdf8" /> Arena de Jogos
        </h2>
        <p style={{ margin: 0, fontSize: '14px', color: '#94a3b8' }}>Onde as amizades vêm para morrer.</p>
        
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginTop: '15px' }}>
          <button onClick={() => setSubTab('marcador')} style={{ background: subTab === 'marcador' ? 'white' : 'rgba(255,255,255,0.1)', color: subTab === 'marcador' ? '#0f172a' : 'white', border: 'none', padding: '8px 16px', borderRadius: '20px', fontWeight: 'bold', cursor: 'pointer', fontSize: '13px' }}>
            🎯 Marcador Rápido
          </button>
          <button onClick={() => setSubTab('torneios')} style={{ background: subTab === 'torneios' ? 'white' : 'rgba(255,255,255,0.1)', color: subTab === 'torneios' ? '#0f172a' : 'white', border: 'none', padding: '8px 16px', borderRadius: '20px', fontWeight: 'bold', cursor: 'pointer', fontSize: '13px' }}>
            🏆 Torneios (Knockout)
          </button>
        </div>
      </div>
        ...
        <>
          <div className="card">
            <h3 style={{ margin: '0 0 12px 0', fontSize: '18px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Activity size={20} color="#38bdf8" /> Novo Jogo Rápido
            </h3>
            <form onSubmit={criarJogo} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                <input type="text" placeholder="Equipa/Jogador A" value={team1} onChange={(e) => setTeam1(e.target.value)} style={{ flex: 1, padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0' }} required />
                <span style={{ fontWeight: 'bold', color: 'var(--text-dim)' }}>VS</span>
                <input type="text" placeholder="Equipa/Jogador B" value={team2} onChange={(e) => setTeam2(e.target.value)} style={{ flex: 1, padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0' }} required />
              </div>
              <button className="btn-primary" disabled={loading} style={{ background: '#0f172a', width: '100%' }}>{loading ? 'A preparar...' : 'Começar Jogo'}</button>
            </form>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            {games.length === 0 ? (
              ...
              games.map((game) => (
                <div key={game.id} className="card" style={{ padding: '0', overflow: 'hidden', border: game.status === 'live' ? '2px solid #38bdf8' : '1px solid #e2e8f0', opacity: game.status === 'live' ? 1 : 0.6 }}>
                  <div style={{ background: game.status === 'live' ? '#f0f9ff' : '#f8fafc', padding: '10px 15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e2e8f0' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span style={{ fontSize: '12px', fontWeight: 'bold', color: game.status === 'live' ? '#0369a1' : '#64748b', textTransform: 'uppercase' }}>{game.status === 'live' ? 'Em Direto' : 'Terminado'}</span>
                    </div>
                    {game.status === 'live' && (
                  ...
                  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'stretch' }}>
                    <div style={{ flex: 1, padding: '20px', textAlign: 'center', borderRight: '1px solid #e2e8f0' }}>
                      ...
                      <div style={{ fontSize: '48px', fontWeight: '900', color: '#0f172a', lineHeight: '1' }}>{game.score1}</div>
                      {game.status === 'live' && (
                        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginTop: '15px' }}>
                          ...
                          <button onClick={() => atualizarScore(game.id, 1, game.score1, 1)} style={{ background: '#38bdf8', border: 'none', width: '40px', height: '40px', borderRadius: '10px', cursor: 'pointer', color: 'white' }}><Plus size={20} /></button>
                        </div>
                      )}
                    </div>
                    <div style={{ flex: 1, padding: '20px', textAlign: 'center' }}>
                      <h4 style={{ margin: '0 0 10px 0', fontSize: '16px', color: 'var(--text)' }}>{game.team2_name}</h4>
                      <div style={{ fontSize: '48px', fontWeight: '900', color: '#0f172a', lineHeight: '1' }}>{game.score2}</div>
                      {game.status === 'live' && (
                        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginTop: '15px' }}>
                          ...
                          <button onClick={() => atualizarScore(game.id, 2, game.score2, 1)} style={{ background: '#38bdf8', border: 'none', width: '40px', height: '40px', borderRadius: '10px', cursor: 'pointer', color: 'white' }}><Plus size={20} /></button>
                        </div>
                      )}
                    </div>
                  </div>
      ...
      {subTab === 'torneios' && (
        <>
          <div className="card">
            <h3 style={{ margin: '0 0 12px 0', fontSize: '18px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Trophy size={20} color="#eab308" /> Criar Torneio
            </h3>
            ...
            <form onSubmit={criarTorneio} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <button className="btn-primary" disabled={loading} style={{ background: '#eab308', width: '100%', color: '#422006' }}>{loading ? 'A gerar...' : 'Gerar Chave (Sorteio)'}</button>
            </form>
          </div>
            ...
            {tournaments.map((tourney) => {
              ...
              return (
                <div key={tourney.id} className="card" style={{ padding: 0, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
                  ...
                  <div style={{ background: tourney.status === 'active' ? '#fefce8' : '#f8fafc', padding: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e2e8f0' }}>
                    <div>
                      <h4 style={{ margin: '0 0 2px 0', fontSize: '18px', color: '#854d0e', display: 'flex', alignItems: 'center', gap: '6px' }}>
                        {tourney.status === 'finished' && <Crown size={18} color="#eab308" />} {tourney.name}
                      </h4>
                  ...
                  <div style={{ padding: '15px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                        ...
                        <div key={rIdx}>
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '10px' }}>
                            {roundMatches.map((match, mIdx) => {
                              ...
                              if (match.p1 === 'BYE' || match.p2 === 'BYE') return null; // Esconde BYEs para ficar clean
                              ...
                              return (
                                <div key={mIdx} style={{ display: 'flex', border: '1px solid #e2e8f0', borderRadius: '8px', overflow: 'hidden', opacity: match.winner ? 0.7 : 1 }}>
                                  <div 
                                    ...
                                    style={{ flex: 1, padding: '10px', textAlign: 'center', background: p1Win ? '#dcfce7' : '#f8fafc', fontWeight: p1Win ? 'bold' : 'normal', cursor: canVote ? 'pointer' : 'default', borderRight: '1px solid #e2e8f0', transition: 'background 0.2s' }}
                                  >
                                    {p1}
                                  </div>
                                  <div style={{ background: '#f1f5f9', width: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', fontWeight: 'bold', color: 'var(--text-dim)' }}>
                                    VS
                                  </div>
                                  <div 
                                    onClick={() => canVote && avancarVencedor(tourney.id, tourney.rounds, rIdx, mIdx, p2)}
                                    style={{ flex: 1, padding: '10px', textAlign: 'center', background: p2Win ? '#dcfce7' : '#f8fafc', fontWeight: p2Win ? 'bold' : 'normal', cursor: canVote ? 'pointer' : 'default', borderLeft: '1px solid #e2e8f0', transition: 'background 0.2s' }}
                                  >
                                    {p2}
                                  </div>
                                </div>
                    ...
                    {tourney.status === 'finished' && (
                      <div style={{ background: '#fef9c3', border: '1px solid #fde047', borderRadius: '12px', padding: '15px', textAlign: 'center', marginTop: '10px' }}>
                        <Crown size={30} color="#ca8a04" style={{ margin: '0 auto 5px auto' }} />
                        <h4 style={{ margin: 0, color: '#854d0e', fontSize: '14px' }}>Grande Vencedor</h4>
                        <p style={{ margin: '5px 0 0 0', fontSize: '20px', fontWeight: '900', color: '#713f12' }}>{tourney.rounds[totalRounds - 1][0].winner}</p>
                      </div>
                    )}
                  </div>
                </div>
      ...
      {confirmModal.show && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(5px)', WebkitBackdropFilter: 'blur(5px)', zIndex: 99999, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px' }}>
          <div className="card" style={{ width: '100%', maxWidth: '320px', textAlign: 'center', margin: 0, padding: '25px 20px', animation: 'scaleIn 0.2s ease-out' }}>
            <h3 style={{ margin: '0 0 10px 0', fontSize: '20px', color: '#0f172a' }}>{confirmModal.title}</h3>
